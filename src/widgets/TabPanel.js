...
/**
 * @class Ext2.TabPanel
 * <p>A basic tab container. TabPanels can be used exactly like a standard {@link Ext2.Panel} for layout
 ...
 * </p>
 * <p><b>Note:</b> TabPanels use their {@link Ext2.Panel#header header} or {@link Ext2.Panel#footer footer} element
 * (depending on the {@link #tabPosition} configuration) to accommodate the tab selector buttons. This means that a TabPanel
 * will not display any configured title, and will not display any configured header {@link Ext2.Panel#tools tools}.
 * To display a header, embed the TabPanel in a {@link Ext2.Panel Panel} which uses
 * <b><tt>{@link Ext2.Container#layout layout: 'fit'}</tt></b>.
 * </p>
 * <p><b>Note:</b> It is advisable to configure all child items of a TabPanel (and any Container which uses a CardLayout) with
 * <b><tt>{@link Ext2.Component#hideMode hideMode: 'offsets'}</tt></b> to avoid rendering errors in child components hidden
 * using the CSS <tt>display</tt> style.
 * </p>
 * <p>There is no actual tab class &mdash; each tab is simply a {@link Ext2.BoxComponent Component} such
 * as a {@link Ext2.Panel Panel}. However, when rendered in a TabPanel, each child Component can fire
...
Ext2.Ajax.request({
    ...
    params: {
        startDate : Ext2.getCmp('start-date').getValue(),
        endDate : Ext2.getCmp('end-date').getValue()
    },
    ...
    failure: function() {
        Ext2.Msg.alert("Grid create failed", "Server communication failure");
    }
});
</code></pre>
 * <p>The server script would need to return an executable Javascript statement which, when processed
 * using <tt>eval()</tt> will return either a config object with an {@link Ext2.Component#xtype xtype},
...
(function() {
    ...
    var grid = new Ext2.grid.GridPanel({
        title: 'Invoice Report',
        bbar: new Ext2.PagingToolbar(store),
        store: store,
        columns: [
...
var tabs = new Ext2.TabPanel({
  ...
  * one or more nested tab divs with class 'x2-tab'). You can also render from markup that is not strictly
  ...
  * that the tab divs in this example contain the class 'x2-hide-display' so that they can be rendered deferred
...
&lt;div id="my-tabs">&lt;/div>
&lt;div id="tab1" class="x2-hide-display">A simple tab&lt;/div>
&lt;div id="tab2" class="x2-hide-display">Another one&lt;/div>
</code></pre>
 * @extends Ext2.Panel
 * @constructor
 * @param {Object} config The configuration options
 */
Ext2.TabPanel = Ext2.extend(Ext2.Panel,  {
    /**
    ...
    monitorResize : true,
    /**
     * @cfg {Boolean} deferredRender Internally, the TabPanel uses a {@link Ext2.layout.CardLayout} to manage its tabs.
     * This property will be passed on to the layout as its {@link Ext2.layout.CardLayout#deferredRender} config value,
     ...
     * a {@link Ext2.form.FormPanel form}, then until a tab is activated, any Fields within that tab will not
     * be rendered, and will therefore not be submitted and will not be available to either
     * {@link Ext2.form.BasicForm#getValues getValues} or {@link Ext2.form.BasicForm#setValues setValues}.</p>
     */
    deferredRender : true,
    /**
    ...
    tabPosition: 'top',
    /**
     * @cfg {String} baseCls The base CSS class applied to the panel (defaults to 'x2-tab-panel').
     */
    baseCls: 'x2-tab-panel',
    /**
     * @cfg {Boolean} autoTabs
     * <p>True to query the DOM for any divs with a class of 'x2-tab' to be automatically converted
     ...
     * divs containing the class 'x2-tab'. To create TabPanels without these limitations, or to pull tab content from
...
&lt;div id="my-tabs">
    &lt;div class="x2-tab" title="Tab 1">A simple tab&lt;/div>
    &lt;div class="x2-tab" title="Tab 2">Another one&lt;/div>
&lt;/div>
</code></pre>
     */
    autoTabs : false,
     ...
     * = true (defaults to 'div.x2-tab').  This can be any valid selector supported by {@link Ext2.DomQuery#select}.
    ...
    initComponent : function(){
        this.frame = false;
        Ext2.TabPanel.superclass.initComponent.call(this);
        this.addEvents(
            /**
        ...
        this.setLayout(new Ext2.layout.CardLayout({
        ...
        if(!this.stack){
            this.stack = Ext2.TabPanel.AccessStack();
        }
        this.initItems();
    },
    ...
    // private
    render : function(){
        Ext2.TabPanel.superclass.render.apply(this, arguments);
    ...
    onRender : function(ct, position){
        Ext2.TabPanel.superclass.onRender.call(this, ct, position);

        if(this.plain){
            var pos = this.tabPosition == 'top' ? 'header' : 'footer';
            this[pos].addClass('x2-tab-panel-'+pos+'-plain');
        }

        var st = this[this.stripTarget];

        this.stripWrap = st.createChild({cls:'x2-tab-strip-wrap', cn:{
            tag:'ul', cls:'x2-tab-strip x2-tab-strip-'+this.tabPosition}});

        var beforeEl = (this.tabPosition=='bottom' ? this.stripWrap : null);
        this.stripSpacer = st.createChild({cls:'x2-tab-strip-spacer'}, beforeEl);
        this.strip = new Ext2.Element(this.stripWrap.dom.firstChild);

        this.edge = this.strip.createChild({tag:'li', cls:'x2-tab-edge'});
        this.strip.createChild({cls:'x2-clear'});

        this.body.addClass('x2-tab-panel-body-'+this.tabPosition);

        if(!this.itemTpl){
            var tt = new Ext2.Template(
                 '<li class="{cls}" id="{id}"><a class="x2-tab-strip-close" onclick="return false;"></a>',
                 '<a class="x2-tab-right" href="#" onclick="return false;"><em class="x2-tab-left">',
                 '<span class="x2-tab-strip-inner"><span class="x2-tab-strip-text {iconCls}">{text}</span></span>',
                 '</em></a></li>'
            );
            tt.disableFormats = true;
            tt.compile();
            Ext2.TabPanel.prototype.itemTpl = tt;
        }
    ...
    afterRender : function(){
        Ext2.TabPanel.superclass.afterRender.call(this);
        if(this.autoTabs){
            this.readTabs(false);
        }
    },
    ...
    // private
    initEvents : function(){
        Ext2.TabPanel.superclass.initEvents.call(this);
    ...
    findTargets : function(e){
        ...
        return {
            close : e.getTarget('.x2-tab-strip-close', this.strip),
            item : item,
            el : itemEl
        };
    },
    ...
    initTab : function(item, index){
        ...
        var cls = item.closable ? 'x2-tab-strip-closable' : '';
        if(item.disabled){
            cls += ' x2-item-disabled';
        }
        if(item.iconCls){
            cls += ' x2-tab-with-icon';
        }
        if(item.tabCls){
            cls += ' ' + item.tabCls;
        }
        ...
        Ext2.fly(el).addClassOnOver('x2-tab-strip-over');

        if(item.tabTip){
            Ext2.fly(el).child('span.x2-tab-strip-text', true).qtip = item.tabTip;
        }
        item.tabEl = el;
    ...
    onBeforeAdd : function(item){
        ...
        Ext2.TabPanel.superclass.onBeforeAdd.apply(this, arguments);
    ...
    onRemove : function(tp, item){
        Ext2.destroy(Ext2.get(this.getTabEl(item)));
    ...
    onItemDisabled : function(item){
        ...
        if(el){
            Ext2.fly(el).addClass('x2-item-disabled');
        }
        this.stack.remove(item);
    },
    ...
    onItemEnabled : function(item){
        var el = this.getTabEl(item);
        if(el){
            Ext2.fly(el).removeClass('x2-item-disabled');
        }
    },
    ...
    onItemTitleChanged : function(item){
        var el = this.getTabEl(item);
        if(el){
            Ext2.fly(el).child('span.x2-tab-strip-text', true).innerHTML = item.title;
        }
    },
    ...
    onItemIconChanged: function(item, iconCls, oldCls){
        var el = this.getTabEl(item);
        if(el){
            Ext2.fly(el).child('span.x2-tab-strip-text').replaceClass(oldCls, iconCls);
        }
    },
    ...
    // private
    onResize : function(){
        Ext2.TabPanel.superclass.onResize.apply(this, arguments);
        this.delegateUpdates();
    },
    ...
    setActiveTab : function(item){
        ...
        if(this.activeTab != item){
            if(this.activeTab){
                ...
                if(oldEl){
                    Ext2.fly(oldEl).removeClass('x2-tab-strip-active');
                }
                this.activeTab.fireEvent('deactivate', this.activeTab);
            }
            var el = this.getTabEl(item);
            Ext2.fly(el).addClass('x2-tab-strip-active');
            this.activeTab = item;
            this.stack.add(item);
    ...
    autoScrollTabs : function(){
        ...
        if(l <= tw){
            ...
            if(!this.scrolling){
                this.pos.addClass('x2-tab-scrolling');
                // See here: http://extjs.com/forum/showthread.php?t=49308&highlight=isSafari
                if(Ext2.isAir || Ext2.isWebKit){
    ...
    createScrollers : function(){
        this.pos.addClass('x2-tab-scrolling-' + this.tabPosition);
        var h = this.stripWrap.dom.offsetHeight;

        // left
        var sl = this.pos.insertFirst({
            cls:'x2-tab-scroller-left'
        });
        sl.setHeight(h);
        sl.addClassOnOver('x2-tab-scroller-left-over');
        this.leftRepeater = new Ext2.util.ClickRepeater(sl, {
        ...
        var sr = this.pos.insertFirst({
            cls:'x2-tab-scroller-right'
        });
        sr.setHeight(h);
        sr.addClassOnOver('x2-tab-scroller-right-over');
        this.rightRepeater = new Ext2.util.ClickRepeater(sr, {
            interval : this.scrollRepeatInterval,
    ...
    scrollToTab : function(item, animate){
        ...
        var left = Ext2.fly(el).getOffsetsTo(this.stripWrap)[0] + pos;
    ...
    updateScrollButtons : function(){
        var pos = this.getScrollPos();
        this.scrollLeft[pos == 0 ? 'addClass' : 'removeClass']('x2-tab-scroller-left-disabled');
        this.scrollRight[pos >= (this.getScrollWidth()-this.getScrollArea()) ? 'addClass' : 'removeClass']('x2-tab-scroller-right-disabled');
    },
    ...
    beforeDestroy : function() {
        if(this.items){
            this.items.each(function(item){
                if(item && item.tabEl){
                    Ext2.get(item.tabEl).removeAllListeners();
                    item.tabEl = null;
                }
            }, this);
        }
        ...
        Ext2.TabPanel.superclass.beforeDestroy.apply(this);
    }
...
Ext2.reg('tabpanel', Ext2.TabPanel);
 ...
 * @method activate
 */
Ext2.TabPanel.prototype.activate = Ext2.TabPanel.prototype.setActiveTab;

// private utility class used by TabPanel
Ext2.TabPanel.AccessStack = function(){
    var items = [];
    return {
        add : function(item){
            items.push(item);
