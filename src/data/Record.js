...
/**
 * @class Ext2.data.Record
 * <p>Instances of this class encapsulate both Record <em>definition</em> information, and Record
 * <em>value</em> information for use in {@link Ext2.data.Store} objects, or any code which needs
 * to access Records cached in an {@link Ext2.data.Store} object.</p>
 * <p>Constructors for this class are generated by passing an Array of field definition objects to {@link #create}.
 * Instances are usually only created by {@link Ext2.data.Reader} implementations when processing unformatted data
 * objects.</p>
 * <p>Note that an instance of a Record class may only belong to one {@link Ext2.data.Store Store} at a time.
 ...
 * This means that a whole Record may not be encoded using {@link Ext2.util.JSON.encode}. Instead, use the
 * {@link data} and {@link id} properties.</p>
 * Record objects generated by this constructor inherit all the methods of Ext2.data.Record listed below.
 * @constructor
 * This constructor should not be used to create Record objects. Instead, use {@link #create} to
 * generate a subclass of Ext2.data.Record configured with information about its consituent fields.
 * @param {Object} data An object, the properties of which provide values for the new Record's fields.
 * @param {Object} id (Optional) The id of the Record. This id should be unique, and is used by the
 * {@link Ext2.data.Store} object which owns the Record to index its collection of Records. If
 * not specified an integer id is generated.
 */
Ext2.data.Record = function(data, id){
    this.id = (id || id === 0) ? id : ++Ext2.data.Record.AUTO_ID;
    this.data = data || {};
};
 ...
 * data types, and a mapping for an {@link Ext2.data.Reader} to extract the field's value from a data object.
 ...
 * for example, the <tt>dataIndex</tt> property in column definition objects passed to {@link Ext2.grid.ColumnModel}</div></li>
 * <li><b>mapping</b> : String/Number<div class="sub-desc">(Optional) A path specification for use by the {@link Ext2.data.Reader}
 * implementation that is creating the Record to select the field value from the data object.<ul>
 * <li>If an {@link Ext2.data.JsonReader} is being used, then this is a string containing the javascript expression to
 * reference the data relative to the Record item's root. Defaults to the field name.</li>
 * <li>If an {@link Ext2.data.XmlReader} is being used, this is an {@link Ext2.DomQuery} path to the data item relative to the
 * DOM element that represents the Record. Defaults to the field name.</li>
 * <li>If an {@link Ext2.data.ArrayReader} is being used, this may be a number indicating the Array index of the field's value.
 ...
 * in order to ensure correct sort ordering. Predefined functions are provided in {@link Ext2.data.SortTypes}.</div></li>
...
var TopicRecord = Ext2.data.Record.create([ // creates a subclass of Ext2.data.Record
 ...
 * to the definition. The constructor has the same signature as {@link #Ext2.data.Record}.
 * @static
 */
Ext2.data.Record.create = function(o){
    var f = Ext2.extend(Ext2.data.Record, {});
    var p = f.prototype;
    p.fields = new Ext2.util.MixedCollection(false, function(field){
        return field.name;
    });
    for(var i = 0, len = o.length; i < len; i++){
        p.fields.add(new Ext2.data.Field(o[i]));
    }
...
Ext2.data.Record.AUTO_ID = 1000;
Ext2.data.Record.EDIT = 'edit';
Ext2.data.Record.REJECT = 'reject';
Ext2.data.Record.COMMIT = 'commit';

Ext2.data.Record.prototype = {
    /**
     * <p><b>This property is stored in the Record definition's <u>prototype</u></b></p>
     * A MixedCollection containing the defined {@link Ext2.data.Field Field}s for this Record.  Read-only.
     * @property fields
    ...
    /**
     * Usually called by the {@link Ext2.data.Store} which owns the Record.
     * Rejects all changes made to the Record since either creation, or the last commit operation.
     * Modified fields are reverted to their original values.
     * <p>
     * Developers should subscribe to the {@link Ext2.data.Store#update} event to have their code notified
     ...
     */
    copy : function(newId) {
        return new this.constructor(Ext2.apply({}, this.data), newId || this.id);
    },
